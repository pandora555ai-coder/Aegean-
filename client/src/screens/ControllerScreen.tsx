...
import { BlitzSwipe } from '../components/BlitzSwipe';
...
export default function ControllerScreen() {
  ...
  // Blitz mode (Task 156). `blitzIndex` is the next statement to show - it
  ...
  // OWN counts and nothing about anyone else.
  ...
  useEffect(() => {
    ...
    // Blitz mode (Task 156) - set together for the same reason applyDraw
    ...
    function handlePhaseChanged(payload: PhaseChangedPayload) {
      ...
      // Blitz (Task 156) - the Task 140 trap: phase-scoped state is cleared
      ...
      // and GAME_OVER clears both in handleGameOver (each after its payload
      // lands, so there is no one-render flash of the waiting view); any
      // other phase arriving means this phone must not still be swiping.
    ...
    // Blitz mode (Task 156) - the player branch of an asymmetric event. The
    ...
    function handleStateSync(payload: StateSyncPayload) {
      ...
      switch (payload.phase) {
        ...
        case 'NUMERIC_REVEAL':
          ...
          setPausedByName(payload.pausedByName);
          break;
        // Blitz mode (Task 156) - same reconnect reasoning: applyBlitz reads
        // how far this phone already got (answeredCount) straight from the
        // server's own state, so a reconnect resumes at the right statement.
        case 'BLITZ':
          if (!isBlitzShowHostPayload(payload)) {
            applyBlitz(payload);
            setPaused(payload.paused);
  ...
  // Blitz mode (Task 156). The surface reports the index it showed; only the
  // statement this phone is actually on is sent, and the phone advances at
  // once - there is no ack, the server stamps and validates on its side
  // (next index only), and the truth never comes back until BLITZ_REVEAL.
  ...
  // Blitz mode (Task 156) - BLITZ. One statement at a time on the swipe
  // surface, n/K above it, a waiting card once all K are swiped. No
  // correctness feedback: the truth isn't on this device.
  if (blitz) {
    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.blitzHeader}>
          <span style={styles.category}>Αστραπή</span>
          <span style={styles.blitzProgress} data-testid="blitz-progress">
            {Math.min(blitzIndex, blitz.total)}/{blitz.total}
          </span>
        </div>
        {finished ? (
          <div style={styles.powerUpLocked} data-testid="blitz-finished">
            <div style={styles.powerUpLockedIcon}>⚡</div>
            <div style={styles.powerUpLockedTitle}>Τέλος!</div>
            <div style={styles.powerUpLockedDetail}>Περίμενε τους υπόλοιπους...</div>
          </div>
        ) : (
          <BlitzSwipe statements={blitz.statements} index={blitzIndex} disabled={paused} onSwipe={handleBlitzSwipe} />
        )}
        <PauseControl paused={paused} pausedByName={pausedByName} onPause={handlePause} onResume={handleResume} />
      </div>
    );
  ...
  // Blitz mode (Task 156) - BLITZ_REVEAL. This phone's own three counts and
  ...
  if (blitzReveal) {
    return (
      <div style={styles.container}>
        ...
        <div style={styles.category}>Αστραπή</div>
...
const styles: Record<string, CSSProperties> = {
  ...
  // Blitz mode (Task 156) - the title/progress strip above the swipe surface.
  blitzHeader: {
    display: 'flex',
    alignItems: 'baseline',
    justifyContent: 'space-between',
