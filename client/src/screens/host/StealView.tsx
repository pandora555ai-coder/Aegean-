...
// stealClampNote (hostStyles.ts) still carry pre-Θέατρο tokens (and,
// for the thief/amount rows, gold) - the text people read here now sits on
// a papyrus panel, so every row reads as plain --carve regardless of thief,
        ...
        <PapyrusPanel className="enter-pop" style={{ flex: '0 0 auto' }} data-testid="steal-resolved">
          <div style={papyrusTextBlockStyle}>
            {resolved.victimName === null ? (
              <>
                <div style={{ ...styles.stealNothing, color: 'var(--carve)' }} data-testid="steal-nothing">
                  Δεν πρόλαβε να διαλέξει!
                </div>
                <div style={{ ...styles.stealClampNote, color: 'var(--carve)' }}>Κανείς δεν έχασε πόντους</div>
              </>
            ) : (
              <>
                ...
                {resolved.stolenAmount < resolved.attemptedAmount && (
                  <div style={{ ...styles.stealClampNote, color: 'var(--carve)' }} data-testid="steal-clamped">
        ...
        <>
          <PapyrusPanel className="enter-pop" style={{ flex: '0 0 auto' }}>
            <div style={papyrusTextBlockStyle}>
              ...
              <div
                style={{ ...styles.stealAmount, color: 'var(--carve)', fontSize: `${(2.25 * s).toFixed(2)}rem` }}
