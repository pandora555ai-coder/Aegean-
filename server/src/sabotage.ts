...
// used by both Task 28a sabotage and Task 30a power-ups. Stacking rules
// (Task 31a) live here and nowhere else:
...
export function applyPendingSabotage(room: Room): void {
  ...
  for (const [targetPlayerId, cast] of room.pendingSabotageByTarget) {
    addAppliedSabotage(room, targetPlayerId, cast.effect);
    ...
    console.log(
      `room ${room.code} sabotage '${cast.effect}' from ${cast.casterName} landed on ${cast.targetName} ` +
        `at ${room.questionStartedAt} (question ${room.currentQuestionIndex + 1}) — stack now ` +
